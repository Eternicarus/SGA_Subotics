 ...
 *
 * @param thread        线程对象
 * @param stack_used    [输出] 已使用栈空间（单位：字节）
 * @param stack_size    [输出] 栈总大小
 * @param stack_peak    [输出] 峰值使用情况（这个可以先不管）
 */
void rt_thread_stack_info(rt_thread_t thread, rt_size_t *stack_used, rt_size_t *stack_size, rt_size_t *stack_peak)
{
    ...
    stack_start = (rt_uint8_t *)thread->stack_addr;
    size = thread->stack_size;

    /* 从栈底部扫描，找到第一个非初始化值（通常是 '#' 或 0xCC） */
    for (i = 0; i < size; i++)
    {
        if (stack_start[i] != '#')   // 这里要匹配初始化时的填充值
        {
            break;
        }
    }
    ...
    if (stack_peak)
        *stack_peak = *stack_used;  // 简单实现，直接返回已使用作为峰值
}
