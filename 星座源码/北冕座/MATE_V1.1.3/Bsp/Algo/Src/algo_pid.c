...
float Algo_PID_Calculate(tagPID_T *_tPid, float _fCurrValue, float _fExpValue)
{
    ...
    float dt = 0.01; //设置10ms为采样周期
    float fCut = 20; //先调节频率为20Hz 响应慢再高
    /* 设定期望值和当前值 */
    ...
    /* 计算未滤波的微分项 */
    float D_raw = _tPid->fKd * (_tPid->fError[0] - 2.0f * _tPid->fError[1] + _tPid->fError[2]);

    /* 计算低通滤波参数 */
    float RC = 1.0f / (2.0f * M_PI * fCut);
    float alpha = dt / (RC + dt);

    /* 应用一阶低通滤波 */
    _tPid->fDout = _tPid->fDout + alpha * (D_raw - _tPid->fDout);
...
void Algo_PID_Clear(tagPID_T *_tPid)
{
    ...
    float compensation = _tPid->fExp_Value - _tPid->fCurr_Value;
    
    _tPid->fError[0] = _tPid->fError[1] = _tPid->fError[2] = 0.0f;

    _tPid->fPre_Value = _tPid->fCtrl_Out + compensation;
